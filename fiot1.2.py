...
# Load tool configurations from JSON or similar for easy expansion
...
def detect_installed_tools(tool_patterns):
    """Detect available tools from the provided patterns."""
    installed_tools = {}
...
def run_exploits_on_all_ips(target_ips, output_file="exploit_results.json", outdated_file="outdated_services.json", use_proxy=False):
    ...
    available_tools = detect_installed_tools(tool_patterns)
    
    with ThreadPoolExecutor(max_workers=5) as executor:
        futures = {
            executor.submit(run_exploits_for_services, ip, discover_services(ip, use_proxy), available_tools, use_proxy): ip
